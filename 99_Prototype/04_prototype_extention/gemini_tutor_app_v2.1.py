#!/usr/bin/env python3
"""
Gemini Tutor CLI v2.1 - Multi-LLM 통합 (Groq/Perplexity/Gemini)
Windows 11 PowerShell 5.1 호환 프로토타입
FIXED: Textual Layout 에러 수정
"""
...
try:
    ...
    from textual.widgets import Header, Footer, Static, Input, TextArea, Label
    from textual.app import App
except ImportError as e:
    print(f"❌ 필수 라이브러리 누락: {e}")
    print("설치 명령: pip install groq google-generativeai rich textual perplexity-python httpx")
    sys.exit(1)
...
class SetupWizard:
    ...
    def run(self):
        """Setup 마법사 실행"""
        self.console.clear()
        self.console.print("[bold cyan]🎯 Gemini Tutor CLI v2.1 - 초기 설정[/bold cyan]\n")
    ...
    def _setup_llm(self):
        """LLM 선택 및 설정"""
        self.console.print("[bold yellow]📡 LLM 제공자 선택 (v2.1)[/bold yellow]")
...
# ============================================================================
# 4. TUI 애플리케이션 (FIXED: Textual Layout 호환성)
# ============================================================================
...
class TerminalPanel(Static):
    ...
    def render(self):
        content = "\n".join(self.history[-20:]) if self.history else "입력을 기다리는 중..."
        return Panel(
            content,
            title="메인 터미널 (입력 창)",
            border_style="green"
...
class GeminiTutorApp(App):
    """메인 TUI 애플리케이션 (FIXED: Textual Layout)"""
